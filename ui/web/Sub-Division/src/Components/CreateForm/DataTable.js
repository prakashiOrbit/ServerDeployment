      ...
      <TableRow>
        ...
        {Object.keys(inputDetails).length
          ? inputDetails.headCells.map((headCell, index) => (
              <TableCell>
                <TableSortLabel>{headCell.headerLabel}</TableSortLabel>
              </TableCell>
            ))
          : null}
      </TableRow>
...
function EnhancedTableToolbar(props) {
  ...
  const { handleDeleteButton, setDeleteState } = props;
...
export default function EnhancedTable({
  ...
  const setDeleteState = async () => {
    const { inputDetails, selected } = this.state;
    const newInputDetails = {
      headCells: inputDetails.headCells,
      data: inputDetails.data.filter((row) => row[0] !== selected[0]),
    };
    this.setState({ inputDetails: newInputDetails, selected: null });
  };
                    ...
                    return (
                      ...
                      >
                        ...
                        <IconButton size="large">
                          <ListAltOutlinedIcon style={{ color: "black" }} />
                        </IconButton>
                      </TableRow>
                    );
                  })
